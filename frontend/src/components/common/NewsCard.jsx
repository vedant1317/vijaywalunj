...
const NewsCard = ({ title, summary, imageUrl, category, publishedAt, sourceUrl }) => {
  ...
  return (
    <div className="group cursor-pointer flex flex-col h-full bg-white relative overflow-hidden transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] border border-ink-100" style={{clipPath:'polygon(0 0, 100% 0, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0 100%)'}}>
      <div className="relative overflow-hidden h-56 bg-cream-dark">
        {imageUrl ? (
          ...
          <div className="w-full h-full bg-ink-900 flex items-center justify-center relative">
        ...
        <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm text-ink-900 text-[10px] uppercase tracking-widest font-bold px-3 py-1.5 border-l-2 border-saffron-600">
          {category || 'Updates'}
        </div>
      </div>
      <div className="p-6 flex-1 flex flex-col relative z-20 bg-white">
        ...
        <h3 className="font-heading font-bold text-ink-900 text-xl leading-[1.3] mb-3 group-hover:text-saffron-600 transition-colors line-clamp-2">{title}</h3>
        <p className="text-ink-600 text-sm font-light leading-relaxed line-clamp-3 mb-6 flex-1">{summary}</p>
        <div className="mt-auto">
          {sourceUrl ? (
            <a href={sourceUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-[10px] uppercase tracking-widest font-bold text-ink-900 group-hover:text-saffron-600 transition-colors">
              <span className="mr-2">Read Article</span> <FiArrowRight className="group-hover:translate-x-1 transition-transform" />
            </a>
          ) : (
            <span className="inline-flex items-center text-[10px] uppercase tracking-widest font-bold text-ink-400">
