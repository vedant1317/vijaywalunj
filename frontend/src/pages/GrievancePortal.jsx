...
const StatusBadge = ({ status }) => {
  const map = {
    'Submitted': 'bg-sand-200 text-ink-700',
    'Under Review': 'bg-yellow-100 text-yellow-700',
    'In Progress': 'bg-orange-100 text-orange-700',
    'Resolved': 'bg-green-100 text-green-700',
    'Closed': 'bg-ink-100 text-ink-600',
  };
  return <span className={`badge-status ${map[status] || 'bg-ink-100 text-ink-600'}`}>{status}</span>;
};
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
            {submitted ? (
              <div className="card p-8 text-center">
                ...
                <h2 className="text-2xl font-bold text-ink-800 mb-2">Grievance Submitted!</h2>
                <p className="text-ink-500 mb-6">Your grievance has been received. Please save your reference ID for tracking.</p>
                <div className="bg-saffron-50 border-2 border-saffron-200 rounded-xl px-8 py-5 inline-block mb-6">
                  ...
                  <div className="text-2xl font-bold text-ink-800 font-mono">{submitted.referenceId}</div>
                </div>
                <p className="text-sm text-ink-500 mb-6">Status: <StatusBadge status={submitted.status} /></p>
              ...
              <div className="card p-6 sm:p-8">
                ...
                <form onSubmit={handleSubmit} className="mt-6 space-y-5">
                  ...
                  <div>
                    <label className="label">Detailed Description * <span className="text-xs text-ink-400 font-normal">(min. 20 characters)</span></label>
                  ...
                  <div>
                    <label className="label">Attachment <span className="text-xs text-ink-400 font-normal">(optional, max 5MB – JPG, PNG, PDF)</span></label>
                    <label className="flex items-center gap-3 cursor-pointer border-2 border-dashed border-ink-300 hover:border-saffron-400 rounded-lg px-4 py-3 transition-colors group">
                      <MdAttachFile className="w-5 h-5 text-ink-400 group-hover:text-saffron-500" />
                      <span className="text-sm text-ink-500">{attachment ? attachment.name : 'Click to attach a file'}</span>
          ...
          <div className="space-y-6">
            ...
            <div className="card p-6">
              <h3 className="font-bold text-ink-800 text-base mb-1">Track Your Grievance</h3>
              ...
              {trackResult && (
                <div className="mt-4 p-4 bg-ink-50 rounded-lg text-sm space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-semibold text-ink-800">{trackResult.referenceId}</span>
                    <StatusBadge status={trackResult.status} />
                  </div>
                  <p className="text-ink-600"><span className="font-medium">Category:</span> {trackResult.category}</p>
                  <p className="text-ink-600"><span className="font-medium">Subject:</span> {trackResult.subject}</p>
                  <p className="text-ink-400 text-xs">Submitted: {new Date(trackResult.createdAt).toLocaleDateString('en-IN')}</p>
                  {trackResult.remarks && <p className="text-ink-600 text-xs mt-1 italic">Remarks: {trackResult.remarks}</p>}
                </div>
              )}
            </div>
            ...
            {/* Process */}
            <div className="card p-6">
              <h3 className="font-bold text-ink-800 text-base mb-4">How it Works</h3>
              {[
                ...
                <div key={step} className="flex items-start gap-3 mb-4 last:mb-0">
                  ...
                  <p className="text-sm text-ink-600 pt-1">{text}</p>
                </div>
              ))}
            </div>
          </div>
