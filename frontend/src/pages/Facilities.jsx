...
const DocumentsList = ({ docs }) => {
  ...
  return (
    <div className="mt-3 border border-ink-200 rounded-lg overflow-hidden">
      <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between px-4 py-2.5 bg-ink-50 text-sm font-semibold text-ink-700 hover:bg-ink-100 transition-colors">
...
const FacilityCard = ({ title, description, type, icon, actionLabel, actionUrl, pdfUrl, externalUrl, prerequisiteDocuments }) => (
  <div className="card p-6 flex flex-col h-full border border-ink-100 hover:border-saffron-200 transition-all duration-300">
    <div className="flex items-start gap-4 mb-4">
      ...
      <div>
        <h3 className="font-bold text-ink-800 text-base leading-tight">{title}</h3>
        <span className="inline-block mt-1 text-xs bg-ink-50 text-ink-600 px-2 py-0.5 rounded-full font-medium capitalize">
          {type.replace(/_/g, ' ')}
        </span>
      </div>
    </div>
    <p className="text-ink-500 text-sm leading-relaxed flex-grow">{description}</p>
...
const Facilities = () => (
  <div className="pt-24 pb-16">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12">
      ...
      {/* Info box */}
      <div className="bg-saffron-50 border border-saffron-200 rounded-xl p-5 flex gap-4 items-start">
        <span className="text-2xl">ℹ️</span>
        <div>
          <h4 className="font-bold text-ink-800 mb-1">Need assistance with a form?</h4>
          <p className="text-sm text-ink-600">Visit the Vijay Walunj office at Sector 10, Vashi during office hours (Mon–Sat, 10 AM – 6 PM) or call us for guidance on form filling and document submission.</p>
        </div>
      </div>
    </div>
  </div>
